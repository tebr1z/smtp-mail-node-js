const LoginForm1 = document.getElementById('loginClick')
const LoginForm2 = document.getElementById('RegisterClick')

LoginForm2.addEventListener('click', function (e) {
    e.preventDefault()
    document.getElementById('login-form1').style.display = 'block'
    document.getElementById('login-form2').style.display = 'none'
})
LoginForm1.addEventListener('click', function (e) {
    e.preventDefault()
...
formEl.addEventListener('submit', function (e) {
    ...
    if (allValid) {
        ...
        .then(response => {
            console.log('E-posta gönderme isteği tamamlandı!', response);
        })
        .catch(error => {
...
FormSinging.addEventListener('submit', function (e) {
    ...
    if (RegisterData2.includes(LoginData)) {
        ...
        document.getElementById('login-form2').style.display = 'none'
        setTimeout(() => {
            CheckLogin.style.width = '0%'
            CheckLogin.style.overflow = 'hidden';
            document.getElementById('login-form2').style.display = 'block'
        }, 3000)
    }
    else {
